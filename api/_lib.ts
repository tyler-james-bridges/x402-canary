// Shared logic for Vercel serverless API functions
...
const EMPTY_X402: X402Details = { version: null, price: null, priceRaw: null, network: null, networkRaw: null, asset: null, assetSymbol: null, payTo: null, accepts: [] };
...
export async function checkEndpoint(url: string, method = "GET"): Promise<CheckResult> {
  ...
  try {
    ...
    if (res.status === 402) {
      ...
      return { url, status: 402, responseTimeMs, timestamp, isHealthy: true, isX402: true, x402Details: paymentHeader ? parsePaymentRequired(paymentHeader) : EMPTY_X402 };
    }
    return { url, status: res.status, responseTimeMs, timestamp, isHealthy: res.status < 500, isX402: false };
  } catch (err) {
    return { url, status: null, responseTimeMs: Date.now() - start, timestamp, isHealthy: false, isX402: false, error: err instanceof Error ? err.message : String(err) };
  }
}
